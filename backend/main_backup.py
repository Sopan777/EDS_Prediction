...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    total_parts = sum(r.parts_checked for r in records)
    total_nok = sum(r.nok for r in records)
    today_records = sum(1 for r in records if r.date == date.today())
    ...
    if user.role == "admin":
        ...
        for auditor, rec_date, count in grouped:
            if auditor not in auditor_daywise:
                auditor_daywise[auditor] = []

            auditor_daywise[auditor].append({
        ...
        auditor_daywise[user.username] = [
            {
                "date": rec_date.strftime("%Y-%m-%d"),
                "count": count
            }
            for rec_date, count in grouped
        ]
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "auditor_daywise": auditor_daywise,
        "show_header": True
    })
...
def analytics_page(request: Request,
    ...
    if analysis_type:
        ...
        for r in records:
            key = getattr(r, analysis_type)
            if key not in grouped:
                grouped[key] = {"parts": 0, "nok": 0}

            grouped[key]["parts"] += r.parts_checked
            grouped[key]["nok"] += r.nok

        labels = list(grouped.keys())
        parts = [grouped[k]["parts"] for k in labels]
        nok = [grouped[k]["nok"] for k in labels]
...
def observations(request: Request,
    ...
    query = db.query(models.Case).join(models.Record)
