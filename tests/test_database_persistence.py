...
==================================
Automated tests for DHATU BODH SQLite persistence layer (database.py)
and rule engine prediction accuracy.
Verifies:
  - Knowledge base singleton caching (12 material families)
  - Microanalysis prediction (Austenitic SS, Bronze, Carbon Steel)
  - Safe abstention on sparse input ({S: 0.2} -> UNKNOWN)
  - PDF report table extraction & scoring
  - Ground-truth reference spectra validation
  - Full CRUD lifecycle for official analysis reports
  - Ratio gate updates and persistence
  - Audit log and user management records
  - Alloy reference presets lifecycle
"""
...
import database
from rule_engine.scoring import Decision, predict_spectrum, get_knowledge_base
from rule_engine.normalize import normalize_spectrum, State
from rule_engine.real_data import load_spectra
from backend.ingestion.eds_geometry import extract_tables as extract_pdf_tables


def test_kb_loaded():
    """Verify KB contains all 12 material families."""
...
def test_pdf_extraction_and_scoring():
    ...
    extracted = extract_pdf_tables(str(pdf_path))
    ...
    pred = predict_spectrum(clean_comp, knowledge=get_knowledge_base())
    assert pred.top is not None
...
def test_reference_dataset_validation():
    ...
    spectra_objs = load_spectra()
    if not spectra_objs:
        pytest.skip("Reference dataset not found")

    assert len(spectra_objs) >= 170
    kb = get_knowledge_base()

    passing = 0
    for s in spectra_objs:
        p = predict_spectrum(s.values, analysed_elements=list(s.values.keys()), knowledge=kb)
        if p.top is not None:
            passing += 1

    pass_rate = passing / len(spectra_objs)
...
def test_sqlite_database_tables():
    """Verify SQLite tables exist and contain required schema."""
    conn = database.get_connection()
    cur = conn.cursor()

    cur.execute("SELECT name FROM sqlite_master WHERE type='table'")
    tables = {row[0] for row in cur.fetchall()}
    assert {"reports", "audit_logs", "ratio_gates", "users", "alloy_presets", "prediction_feedback"}.issubset(tables)
...
def test_database_reports_crud():
    ...
    report_id = database.save_report(
        title="Microanalysis QA Certificate #999",
        sample_id=sample_id,
        lot_number="LOT-2026-X01",
        customer="Bosch Plant 1",
        analyst_id="usr-current",
        analyst_name="Lab Metallurgist",
        source_type="file_upload",
        source_filename="specimen_test.pdf",
        raw_composition=raw_comp,
        normalized_composition=norm_comp,
        decision="identified",
        family_id="F4",
        family_label="Austenitic stainless steel 18/8",
        grade_hint="X8CrNiS18-9 / AISI 304",
        compatibility_pct=95,
        candidates=["Back Flow Tube", "Locking Sleeve"],
        caveats=["Renormalized excluding C, O."],
        analyst_notes="Verified against calibration standard.",
    ...
    assert rep["sample_id"] == sample_id
    assert rep["family_id"] == "F4"
    assert rep["status"] == "Final Sign-off"

    # Update status
    database.update_report_status(report_id, "Archived", "Lab Metallurgist")
    rep_updated = database.get_report_by_id(report_id)
    assert rep_updated["status"] == "Archived"

    # Verify present in report list
    all_reps = database.get_reports(search_query=sample_id)
    assert any(r["id"] == report_id for r in all_reps)

    # Delete report
    deleted = database.delete_report(report_id, user_name="Lab Metallurgist")
    assert deleted is True
    assert database.get_report_by_id(report_id) is None


def test_database_ratio_gates_lifecycle():
    """Verify ratio gate saving, retrieval, and resetting."""
    family_id = "F4"
    custom_gates = [
        {
            "id": "gate-test-cr-ni",
    ...
    # Save
    database.save_gates_for_family(family_id, custom_gates, user_name="Lab Metallurgist")
    ...
    # Reset
    database.reset_gates_for_family(family_id, user_name="Lab Metallurgist")
...
def test_database_user_and_audit_logging():
    ...
    user_id = database.add_user(
        name="Chief Metallurgist",
        email="metallurgy@dhatubodh.io",
    ...
    audit_id = database.log_audit(
        user_id=user_id,
        user_name="Chief Metallurgist",
        user_role="Chief Metallurgist",
...
def test_database_alloy_presets():
    """Verify alloy reference templates retrieval and creation."""
